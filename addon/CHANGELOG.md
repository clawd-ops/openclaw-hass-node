...
## 2026.6.19b2 (2026-06-20) — Assist streaming follow-up fixes + slow-turn keepalive
...
- **#129 Codex follow-up — close the post-ack stale-trailer window.** The original #128 fix closed the pre-ack window but the post-ack window was still open: after turn-N+1's `chat.send` ack installed the new `active_run`, a `session.message` arriving from the prior turn's broker queue without a `runId` was still accepted (the `_terminal_assistant_text` guard had just been cleared by the new turn-install). It populated the new turn's terminal buffer and woke the fast-path waiter with stale text. Closed by tracking, per session, the `runId` for which at least one event has been accepted; on follow-up turns a `runId`-less `session.message` is only accepted once we've seen a `runId`-tagged event for the current `active_run`. Scoped to follow-up turns (`_has_prior_terminal` set) so the first turn — where some gateway flows legitimately emit only a `runId`-less `session.message` terminal — keeps working. New regression tests: `test_post_ack_runid_less_stale_session_message_dropped` exercises the exact `ack(new) → stale session.message(no runId) → real reply` ordering; `test_first_turn_runid_less_session_message_accepted` pins the first-turn legitimate path.
- **#129 follow-up — slow-turn keepalive deltas keep HA's stream alive.** HA Assist enforces a ~30s read timeout on the conversation stream. On tool-heavy turns the gateway can stay silent for tens of seconds between `chat.send` ack and the first assistant delta while it runs tool calls, so HA closes the stream early and the user sees "node not responding" even though the real reply lands later (user-reported regression — short conversational turns worked fine, "what's my schedule" / "how do the tanks look" did not). `ChatRelay.stream_turn` now emits a short progress delta (`🔧 Working on it…`) after `_STREAM_FIRST_KEEPALIVE_S` (8s) of silence, then `…still working` every `_STREAM_KEEPALIVE_INTERVAL_S` (15s) thereafter. A real assistant delta resets the timer so fast turns never see a keepalive. New tests: `test_stream_turn_emits_keepalive_during_silent_gap`, `test_stream_turn_no_keepalive_on_fast_turn`.
