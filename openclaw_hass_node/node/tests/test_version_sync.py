...
def _read_pyproject_version() -> str:
    raw = (_REPO_ROOT / "openclaw_hass_node" / "node" / "pyproject.toml").read_bytes()
...
def _read_init_fallback() -> str:
    ...
    """
    text = (
        _REPO_ROOT / "openclaw_hass_node" / "node" / "src" / "openclaw_node" / "__init__.py"
...
def _read_addon_config_version() -> str:
    text = (_REPO_ROOT / "openclaw_hass_node" / "config.yaml").read_text(encoding="utf-8")
...
def _read_build_yaml_version() -> str:
    text = (_REPO_ROOT / "openclaw_hass_node" / "build.yaml").read_text(encoding="utf-8")
