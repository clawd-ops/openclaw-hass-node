...
## 2026.6.20b9 (2026-06-27) — Rename add-on directory to match slug (Supervisor discovery fix)

### Fixes
- **Renamed the add-on directory from `addon/` to `openclaw_hass_node/`.**
  Newer HA Supervisor identifies store entries by directory name and
  reconciles them against installed instances by `<repo-hash>_<dir-name>`.
  The previous layout had `dir-name="addon"` but `slug="openclaw_hass_node"`,
  which caused the installed instance to become orphaned in the store
  after b6 — Supervisor pulled new commits cleanly but could not match
  them to the installed app (UI showed `App fcccfbbd_openclaw_hass_node
  does not exist in the store`, no icon, no Update prompt). Renaming
  the directory to match the slug closes that gap. Existing installs
  will need a one-time uninstall + reinstall to come up under the
  matched name.

### Internal
- CI workflows, `scripts/bump-version.py`, and `test_version_sync.py`
  updated to read the new `openclaw_hass_node/` paths.
