"""Home Assistant scene configuration command (``ha.config.scene``).

HA-native REST path for per-id scene config under
``/api/config/scene/config/<id>``. This command talks to HA's REST
...
- ``save`` — write one scene (proposal-gated).
- ``delete`` — delete one scene (proposal-gated).
...
def _require_proposal(params: dict[str, Any], action: str) -> dict[str, Any] | None:
    """Enforce proposal gating on mutating scene actions.
    ...
    """
    label = f"ha.config.scene action={action}"
...
async def _action_get(params: dict[str, Any]) -> dict[str, Any]:
    scene_id, err = _require_id(params)
    if err is not None:
        return err
    try:
        result = await ha_get(f"/api/config/scene/config/{scene_id}")
    except HAClientError as exc:
        return _to_error(exc)
    if not isinstance(result, dict):
        return _error(
            "HA_BAD_RESPONSE",
            "Expected dict from /api/config/scene/config/<id>",
        )
    return {"ok": True, "id": scene_id, "config": result}
...
async def handle_ha_config_scene(params: dict[str, Any]) -> dict[str, Any]:
    """Dispatch a scene config action.
