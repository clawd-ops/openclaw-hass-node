"""Tests for openclaw_node.commands.ha_config_scene."""
...
async def test_missing_action() -> None:
    result = await handle_ha_config_scene({})
...
async def test_action_wrong_type(bad_action: object) -> None:
    result = await handle_ha_config_scene({"action": bad_action})
    assert result["ok"] is False
    assert result["error"] == "INVALID_PARAM"


async def test_action_empty_string() -> None:
    result = await handle_ha_config_scene({"action": "   "})
    assert result["error"] == "INVALID_PARAM"


async def test_unknown_action() -> None:
    result = await handle_ha_config_scene({"action": "purge"})
    assert result["error"] == "INVALID_PARAM"


# ---------------------------------------------------------------------------
# no action=list — HA does not expose /api/config/scene/config as a
# collection route. Enumeration is via `scene.*` entities from state.
# ---------------------------------------------------------------------------


async def test_list_action_rejected() -> None:
    """`action=list` must be rejected; enumeration goes through state."""
    result = await handle_ha_config_scene({"action": "list"})
    assert result["ok"] is False
    assert result["error"] == "INVALID_PARAM"
...
async def test_get_happy_path() -> None:
    ...
    with patch("openclaw_node.commands.ha_config_scene.ha_get", mock):
        result = await handle_ha_config_scene({"action": "get", "id": "42"})
    assert result == {"ok": True, "id": "42", "config": config}
    mock.assert_awaited_once_with("/api/config/scene/config/42")


async def test_get_missing_id() -> None:
    result = await handle_ha_config_scene({"action": "get"})
...
async def test_get_id_slug_validation(bad_id: str) -> None:
    result = await handle_ha_config_scene({"action": "get", "id": bad_id})
    assert result["error"] == "INVALID_PARAM"


async def test_get_empty_id() -> None:
    result = await handle_ha_config_scene({"action": "get", "id": "   "})
    assert result["error"] == "MISSING_PARAM"
...
async def test_save_missing_proposal_id() -> None:
    result = await handle_ha_config_scene({"action": "save", "id": "1", "config": {"alias": "x"}})
...
async def test_save_missing_config() -> None:
    result = await handle_ha_config_scene({"action": "save", "id": "1", "proposal_id": "p1"})
    assert result["error"] == "MISSING_PARAM"
...
async def test_save_happy_path() -> None:
    ...
    with patch("openclaw_node.commands.ha_config_scene.ha_post", mock):
        result = await handle_ha_config_scene(
            {"action": "save", "id": "42", "config": config, "proposal_id": "p1"}
        )
    assert result == {"ok": True, "id": "42", "proposal_id": "p1"}
    mock.assert_awaited_once_with("/api/config/scene/config/42", config)
...
async def test_delete_missing_proposal_id() -> None:
    result = await handle_ha_config_scene({"action": "delete", "id": "1"})
    assert result["error"] == "PROPOSAL_REQUIRED"


async def test_delete_empty_proposal_id() -> None:
    result = await handle_ha_config_scene({"action": "delete", "id": "1", "proposal_id": "   "})
    assert result["error"] == "PROPOSAL_REQUIRED"


async def test_delete_direct_proposal_id_refused() -> None:
    result = await handle_ha_config_scene({"action": "delete", "id": "1", "proposal_id": "direct"})
    assert result["error"] == "PROPOSAL_REQUIRED"


async def test_delete_non_string_proposal_id() -> None:
    result = await handle_ha_config_scene({"action": "delete", "id": "1", "proposal_id": 123})
    assert result["error"] == "PROPOSAL_REQUIRED"


async def test_delete_missing_id() -> None:
    result = await handle_ha_config_scene({"action": "delete", "proposal_id": "p1"})
    assert result["error"] == "MISSING_PARAM"


async def test_delete_invalid_id_type() -> None:
    result = await handle_ha_config_scene({"action": "delete", "id": 42, "proposal_id": "p1"})
    assert result["error"] == "MISSING_PARAM"


async def test_delete_happy_path() -> None:
    mock = AsyncMock(return_value={"result": "ok"})
    with patch("openclaw_node.commands.ha_config_scene.ha_delete", mock):
        result = await handle_ha_config_scene({"action": "delete", "id": "42", "proposal_id": "p1"})
    assert result == {"ok": True, "id": "42", "proposal_id": "p1"}
    mock.assert_awaited_once_with("/api/config/scene/config/42")
...
async def test_delete_ha_error_propagates() -> None:
    mock = AsyncMock(side_effect=HAClientError("HA_NOT_FOUND", "gone"))
    with patch("openclaw_node.commands.ha_config_scene.ha_delete", mock):
        result = await handle_ha_config_scene({"action": "delete", "id": "1", "proposal_id": "p1"})
    assert result["error"] == "HA_NOT_FOUND"
