...
async def test_recv_challenge_wrong_event() -> None:
    ...
    ws = AsyncMock()
    ws.recv = AsyncMock(
        return_value=json.dumps({"type": "event", "event": "something.else"})
    )
    with pytest.raises(ValueError, match="Expected connect.challenge"):
...
async def test_recv_connect_response_wrong_type() -> None:
    ...
    ws = AsyncMock()
    ws.recv = AsyncMock(
        return_value=json.dumps({"type": "event", "event": "something"})
    )
...
async def test_handle_invoke_ping() -> None:
    ...
    ws = AsyncMock()
    ws.send = AsyncMock()
    await client._handle_invoke(ws, {"invokeId": "inv-1", "command": "ping", "params": {"message": "hi"}})
    ws.send.assert_called_once()
    sent = json.loads(ws.send.call_args[0][0])
...
async def test_handle_invoke_unknown_command() -> None:
    ...
    ws = AsyncMock()
    ws.send = AsyncMock()
    await client._handle_invoke(ws, {"invokeId": "inv-2", "command": "does.not.exist", "params": {}})
    ws.send.assert_called_once()
    sent = json.loads(ws.send.call_args[0][0])
...
async def test_pull_pending_empty_items() -> None:
    ...
    ws.send = AsyncMock()
    ws.recv = AsyncMock(
        return_value=json.dumps({"ok": True, "payload": {"items": []}})
    )
...
async def test_pull_pending_with_items() -> None:
    ...
    ws.recv = AsyncMock(
        return_value=json.dumps({"ok": True, "payload": {"items": items}})
    )
    await client._pull_pending(ws)
...
async def test_pull_pending_failure_logged() -> None:
    ...
    ws.send = AsyncMock()
    ws.recv = AsyncMock(
        return_value=json.dumps({"ok": False, "error": "SOME_ERROR"})
    )
...
async def test_event_loop_dispatches_invoke() -> None:
    ...
    messages = [
        json.dumps({"type": "event", "event": "node.invoke.request",
                    "payload": {"invokeId": "e1", "command": "ping", "params": {}}}),
    ]

    async def _recv_iter():
        for m in messages:
            yield m
...
async def test_run_reconnects_on_exception() -> None:
    ...
    with patch.object(client, "_connect_and_loop", _failing_connect_and_loop), \
