...
# ---- runtime hook wiring (P5.4) ----


def _make_client_with_runtime() -> tuple[GatewayClient, Any]:
    """Build a GatewayClient with a NodeRuntime attached."""
...
def test_register_runtime_hooks_sets_forwarder_and_connected() -> None:
    client, runtime = _make_client_with_runtime()
    ws = MagicMock()
    client._register_runtime_hooks(ws)
    assert runtime.gateway_connected is True
    assert runtime.conversation_forwarder is not None
    assert client._conversation is not None


def test_register_runtime_hooks_noop_without_runtime() -> None:
    client = _make_client()
    ws = MagicMock()
    client._register_runtime_hooks(ws)
...
def test_teardown_runtime_hooks_clears_state() -> None:
    client, runtime = _make_client_with_runtime()
    ws = MagicMock()
    client._register_runtime_hooks(ws)
    client._teardown_runtime_hooks(reason="testing")
    assert runtime.gateway_connected is False
    assert runtime.conversation_forwarder is None
    assert client._conversation is None


def test_teardown_runtime_hooks_safe_without_runtime() -> None:
    client = _make_client()
    client._teardown_runtime_hooks(reason="no-op")  # must not raise


async def test_register_send_callback_emits_conversation_request() -> None:
    """The dispatcher's send callback wraps the payload in a node.conversation.request frame."""
    client, _ = _make_client_with_runtime()
    ws = AsyncMock()
    ws.send = AsyncMock()
    client._register_runtime_hooks(ws)

    # Trigger forward() to call send via the dispatcher, then immediately resolve.
    dispatcher = client._conversation
    assert dispatcher is not None
    fwd_task = asyncio.create_task(dispatcher.forward("hi", None, None))
    await asyncio.sleep(0.01)
    sent_raw = ws.send.call_args[0][0]
    sent = json.loads(sent_raw)
    assert sent["type"] == "req"
    assert sent["method"] == "node.conversation.request"
    assert sent["params"]["text"] == "hi"
    # Complete the future so the task finishes cleanly.
    dispatcher.handle_result({"conversationId": sent["params"]["conversationId"], "response": "ok"})
    result = await fwd_task
    assert result["response"] == "ok"


async def test_event_loop_routes_conversation_result_to_dispatcher() -> None:
    """A node.conversation.result event completes the matching pending forward."""
    client, _ = _make_client_with_runtime()
    ws = AsyncMock()
    ws.send = AsyncMock()
    client._register_runtime_hooks(ws)
    ...
    await asyncio.sleep(0.01)
    sent = json.loads(ws.send.call_args[0][0])
    request_id = sent["params"]["conversationId"]

    messages = [
        json.dumps(
            {
                "type": "event",
                "event": "node.conversation.result",
                "payload": {"conversationId": request_id, "response": "routed"},
            }
        ),
    ]
...
async def test_event_loop_ignores_conversation_result_without_dispatcher() -> None:
    """Conversation result before the dispatcher is registered is dropped silently."""
    client = _make_client()  # no runtime, no dispatcher
    ws = AsyncMock()
    ws.send = AsyncMock()
    ...
    ws.__aiter__ = lambda self: _recv_iter().__aiter__()
    await client._event_loop(ws)  # must not raise
