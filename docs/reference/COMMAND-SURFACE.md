...
| `ha.list_automations`     | `include_traces?`, `entity_filter?` (fnmatch glob scoped to `automation.`), `state_filter?` (exact match); unknown params rejected; narrowing applied before trace lookup |
