...
| `fs.restore`  | `path`, exactly one of `version_id`, `version`, `proposal_id`, or `at`, `actor?`, `agent_bridge?` | Restores from the content-addressed **backup store** (not the trash directory). `version_id` is the preferred selector: it is the opaque identifier returned by `fs.history` and also accepted by `fs.diff`, so callers never translate a list position into an index. `version` remains supported and is **1-indexed** (`-1` = latest); `0` is rejected as out of range. Also purges any lingering OpenClaw fallback-trash entries whose full-path slug matches `path` (basename alone is insufficient); returns `trash_purged` count. |
| `fs.history`  | `path`                              | Content-addressed backup history for `path`. Each entry carries a `version_id` (sha256 hex digest), the canonical version identifier accepted by `fs.restore` and `fs.diff`. First-write of a fresh file records no entry (there is nothing to restore to). |
