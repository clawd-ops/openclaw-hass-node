...
| `ha.logbook`              | `entity_id?`, `start_time?`, `end_time?` (REST). **Known defect:** values are not percent-encoded. `end_time` is a query value where `+` means a space, so use the `Z` form there; `start_time` is a path segment where `+` is literal and is not known to fail, but was not separately probed. The Assist tool accepts `start`/`end` and maps them, but a direct invoke must use `start_time`/`end_time` or the bound is silently dropped |
| `ha.history`              | `entity_ids?` (list), `start_time?`, `end_time?`, `minimal_response?`, `no_attributes?`, `significant_changes_only?` (REST). **Known defects:** values are not percent-encoded. `end_time` is a query value where `+` means a space, so a `+00:00` offset is rejected by HA and the `Z` form must be used; `start_time` is a path segment where `+` is literal and is not known to fail, but was not separately probed. An unknown entity returns `{ok: true, count: 0}`, indistinguishable from real empty history. The Assist tool accepts `entity_id`/`start`/`end` and maps them, but a direct invoke must use the node names or the filter and bounds are silently dropped |
