...
| `system.execApprovals.get` | advertised<br>CODE-PROVEN:pass | advertised-unverified<br>CODE-PROVEN:unverified<br>PRODUCTION-LIVE:refused-as-designed | path-present-unverified<br>UNVERIFIED:unverified | unavailable<br>CODE-PROVEN:refused-as-designed | `operator_approval` | `PRODUCTION-LIVE` | **`refused-as-designed`** |
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 read-only completion sweep confirmed fs.glob through direct nodes.invoke.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 read-only completion sweep confirmed fs.history through direct nodes.invoke; the inspected file had no stored versions.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 read-only completion sweep confirmed a bounded binary range read through direct nodes.invoke. Existing test evidence continues to cover fail-closed offset behavior.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 direct nodes.invoke returned the requested configuration.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 direct nodes.invoke returned the selected config entry.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 direct nodes.invoke returned one registered entity.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 direct nodes.invoke returned the requested helper collection.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 direct nodes.invoke returned the dashboard collection.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `partial`**
- Evidence note: The command path was exercised on Sept 13, but this YAML-mode installation returned HA_CONFIG_NOT_FOUND for the default dashboard.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `pass`**
- Evidence note: Sept 13 direct nodes.invoke returned the resource collection.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
- Assist caller: Assist voice turns intentionally do not expose HA-native configuration commands.
...
- Capability conditions: Returns the Gateway-native file snapshot with path, existence, exact raw-byte hash, and a fail-closed policy when storage is missing or malformed. The socket token is redacted from every response. Like system.run, direct nodes.invoke system.execApprovals.get is refused by the Gateway with INVALID_REQUEST; the command is only reachable through the operator exec-approval surface (exec.approvals.node.*). The node advertises it, and it must stay in gateway.nodes.commands.allow, because that pair is what permits the Gateway to forward the command on that operator surface at all; advertisement states what the node can handle when forwarded, not which Gateway entry points may reach it.
- Semantic result: UNVERIFIED CONTRACT: handler-specific result dictionary; no normalized per-command result schema is enforced yet.
- Semantic errors: UNVERIFIED CONTRACT: handler-specific semantic error dictionary; stacked PR #267 preserves it separately from Gateway and transport errors.
- Evidence method: `PRODUCTION-LIVE`
- **Outcome: `refused-as-designed`**
- Evidence note: Sept 13 second-pass live sweep reached the Gateway; direct nodes.invoke refused this advertised command and directed callers to exec.approvals.node.*. Resolved in #333: the refusal is the intended gating and the advertisement is also correct, so neither the node.invoke allowlist nor the advertised command list changes. This command is operator_approval class and mirrors system.run, which is likewise advertised, likewise allowlisted, and likewise refused on direct nodes.invoke. Reaching it through nodes.invoke would let any node-invoke caller read the exec-approval policy document outside the approval flow that governs it. The gap was documentation only, now stated in docs/reference/COMMAND-SURFACE.md.
- Advertisement: Present in the node connect frame; gateway allowlisting and runtime availability are separate.
- Direct caller: A dispatcher and advertised path exist; end-to-end availability is not implied.
- Handler/dispatch: A registered handler exists. Behavioral evidence comes from curated handler/dispatch_async tests, not live Gateway nodes.invoke.
