...
| `openclaw-hass-node-assist-tools`     | The OpenClaw **gateway plugin**. Declares scoped tools (`ha_call_service`, `ha_get_state`, etc.) that wrap `ha.*` node commands so Assist sessions can operate the bound HA node. The plugin does not surface `fs.*` or `system.*`; those command families remain reachable only via `nodes.invoke` in non-Assist sessions. **Assist-only**: every other OC session (chat, cron, sub-agent) has the operator-only `nodes.invoke` tool and uses the `openclaw-hass-node-skill` skill on top of it. | This repo, `plugins/openclaw-hass-node-assist-tools/` (new). |
