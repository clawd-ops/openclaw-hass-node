...
| `openclaw-hass-node-skill`            | The companion **skill** used by every non-Assist session (chat, cron, sub-agent) to drive `ha.*` / `fs.*` via `nodes.invoke`. | OpenClaw skill registry (currently registered as `home-assistant-node`; rename to `openclaw-hass-node-skill` on next skill touch for full symmetry). |
