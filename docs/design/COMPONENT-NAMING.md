...
| `openclaw-hass-node-skill`            | The companion **skill** used by every non-Assist session (chat, cron, sub-agent) to drive `ha.*` / `fs.*` via `nodes.invoke`. | This repo, `skills/home-assistant-node/` today; renames to `skills/openclaw-hass-node-skill/` in the same naming pass (with the skill_workshop proposal to update the installed registry entry). |
...
fresh setup is acceptable. Future external users will only ever see the new
domain.
